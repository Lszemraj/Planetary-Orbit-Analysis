...
ephemeris("/Users/Lillie/Desktop/CL Astro/venus_final.txt")

venus = vector_ephemeris_to_dataframe("/Users/Lillie/Desktop/CL Astro/venus_final.txt")

days = generate_days(venus)
distance = generate_distance(days)
velocity = generate_velocity(distance)
e = generate_eccentricity(velocity)

print("Eccentricity of Venus:", e)
...
fig_v_t = px.scatter(df, x = df["days"], y = df["v"],
                     labels={
                         "days": "Time (Days)",
                         "v": "Velocity (AU/day)" },
                     title="Venus-Sun Velocity Time Graph")
...
fig_d_t = px.scatter(df, x = df["days"], y = df["d"],
                     labels={
                         "days": "Time (Days)",
                         "d": "Distance (AU)" },
                     title="Venus-Sun Distance Time Graph"
                     )
#fig_d_t.show()
...
fig3.update_layout(
    title="Orbit of Venus about the Sun",
    xaxis_title="X (AU)",
    yaxis_title="Y (AU)",
