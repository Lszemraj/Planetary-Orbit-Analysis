...
ephemeris("/Users/Lillie/Desktop/CL Astro/mars_final2.txt")

mars = vector_ephemeris_to_dataframe("/Users/Lillie/Desktop/CL Astro/mars_final2.txt")

days = generate_days(mars)
distance = generate_distance(days)
velocity = generate_velocity(distance)
e = generate_eccentricity(velocity)

print("Eccentricity of Mars:", e)
...
fig_v_t = px.scatter(df, x = df["days"], y = df["v"],
                     labels={
                         "days": "Time (Days)",
                         "v": "Velocity (AU/day)" },
                     title="Mars-Sun Velocity Time Graph")
...
fig_d_t = px.scatter(df, x = df["days"], y = df["d"],
                     labels={
                         "days": "Time (Days)",
                         "d": "Distance (AU)" },
                     title="Mars-Sun Distance Time Graph"
                     )
#fig_d_t.show()
...
fig3.update_layout(
    title="Orbit of Mars about the Sun",
    xaxis_title="X (AU)",
    yaxis_title="Y (AU)",
