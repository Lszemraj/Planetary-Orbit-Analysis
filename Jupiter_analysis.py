...
ephemeris("/Users/Lillie/Desktop/CL Astro/jupiter_final2.txt")

jupiter = vector_ephemeris_to_dataframe("/Users/Lillie/Desktop/CL Astro/jupiter_final2.txt")

days = generate_days(jupiter)
distance = generate_distance(days)
velocity = generate_velocity(distance)
e = generate_eccentricity(velocity)

print("Eccentricity of Jupiter:", e)
...
fig_v_t = px.scatter(df, x = df["days"], y = df["v"],
                     labels={
                         "days": "Time (Days)",
                         "v": "Velocity (AU/day)" },
                     title="Jupiter-Sun Velocity Time Graph")
...
fig_d_t = px.scatter(df, x = df["days"], y = df["d"],
                     labels={
                         "days": "Time (Days)",
                         "d": "Distance (AU)" },
                     title="Jupiter-Sun Distance Time Graph"
                     )
#fig_d_t.show()
...
fig3.update_layout(
    title="Orbit of Jupiter about the Sun",
    xaxis_title="X (AU)",
    yaxis_title="Y (AU)",
