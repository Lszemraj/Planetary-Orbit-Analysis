...
ephemeris("/Users/Lillie/Desktop/CL Astro/uranus_final.txt")

uranus = vector_ephemeris_to_dataframe("/Users/Lillie/Desktop/CL Astro/uranus_final.txt")

days = generate_days(uranus)
distance = generate_distance(days)
velocity = generate_velocity(distance)
e = generate_eccentricity(velocity)

print("Eccentricity of Uranus:", e)
...
fig_v_t = px.scatter(df, x = df["days"], y = df["v"],
                     labels={
                         "days": "Time (Days)",
                         "v": "Velocity (AU/day)" },
                     title="Uranus-Sun Velocity Time Graph")
...
fig_d_t = px.scatter(df, x = df["days"], y = df["d"],
                     labels={
                         "days": "Time (Days)",
                         "d": "Distance (AU)" },
                     title="Uranus-Sun Distance Time Graph"
                     )
#fig_d_t.show()
...
fig3.update_layout(
    title="Orbit of Uranus about the Sun",
    xaxis_title="X (AU)",
    yaxis_title="Y (AU)",
